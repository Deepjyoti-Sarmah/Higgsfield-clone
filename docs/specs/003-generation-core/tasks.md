...
- [x] **T-003-1** · Contract + migrations: settings, models, migrations 0002/0003, domain rules + preset catalog, storage + model Protocols, S3 adapter, test fixtures
...
- [x] **T-003-2** · Repositories: presets, assets, ledger, jobs (+ NOTIFY), job_steps (claim SQL, lease, reaper SQL), user row lock
...
- [x] **T-003-3** · Presets, uploads and credits endpoints + the 60-credit guest grant
...
- [x] **T-003-4** · Jobs API: create (one transaction, idempotent, 402), read, SSE events (LISTEN broker + stream)
...
- [x] **T-003-5** · Worker: claim loop, lease renewal, success/failure completion, reaper
...
- [x] **T-003-7** · End-to-end smoke on local compose (api + worker + MinIO + local-motion) and STATUS lines for AC-1..AC-8
