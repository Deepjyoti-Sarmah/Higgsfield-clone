...
| T-030 | Own preset preview media; delete all hotlinked higgsfield.ai/cloudfront URLs | DONE (deepseek-flash via DSH): 12 x 720p own clips in R2, `preview_key` + migration 0005, hotlinks gone | deepseek-flash | `docs/tasks/T-030/report.md` |
| T-031 | Density pass: media-first tiles, tighter grid, slimmer chrome (visual only) | DONE (deepseek-flash via DSH): full-width 5-up gallery, whole-tile links, 8 tiles above the fold at 1440x900 | deepseek-flash | `docs/tasks/T-031/report.md` |
...
| T-011 | Live slice verification per spec (AC walk + `docs/verification/` screenshots; curl substitute if no browser) | DONE (deepseek-flash via DSH): Playwright + Chromium walk of all 6 pages, 7/7 PASS, screenshots committed | deepseek-flash | `docs/verification/VERIFICATION-REPORT.md` |
...
| T-012 | Modal LTX-2.5 spike: run `apps/gpu/ltx_spike.py` until a real clip lands in R2; record cost + function ref | DONE: real clip in R2 (800,917 B h264, 200 `video/mp4`); A10G, load 27.5 s + generate 339.8 s, ~$0.20 over 5 runs; app `ap-fo98E8fzUL3Y6Y5xW4u3im` | opencode / Muse Spark | `apps/gpu/ltx_spike.py`, `docs/tasks/T-012/` |
| T-013 | Real `ModalAdapter` behind the frozen `ModelAdapter` protocol (server-side only, no contract change) | DONE: adapter live on H100, one clip in 145.2 s (missed ≤120 s; knob 3 open); ~$0.23 paid; default still `local-motion` | opencode / Muse Spark | `apps/api/app/adapters/modal_adapter.py`, `apps/gpu/ltx_spike.py`, `docs/tasks/T-013/` |
...
| T-041 | Create video session-history tiles: no hover state, weak active state, no scroll/focus on open | DONE (Claude Opus 5): border+ring+caption for active, hover lift, play glyph on succeeded, scroll+focus on tile click (any viewport) | Claude Opus 5 | `docs/tasks/T-041/report.md` |
| T-040 | Library: selected row indistinguishable from hover; result opens off-screen with no feedback | DONE (Claude Opus 5): edge+tint+chip selection, scroll+focus+aria-live on select, play glyph on video thumbnails; Create-video history strip flagged with the same weak-selection issue, not fixed (out of scope) | Claude Opus 5 | `docs/tasks/T-040/report.md` |
