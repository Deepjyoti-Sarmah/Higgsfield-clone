...
Needs `modal token new` and `modal secret create r2 R2_ENDPOINT_URL=.. R2_ACCESS_KEY_ID=.. R2_SECRET_ACCESS_KEY=.. R2_BUCKET=..`.
"""
import os
import time
import uuid
...
gpu_image = (
    ...
    .pip_install("torch==2.8.0", "transformers", "accelerate", "sentencepiece", "av", "boto3",
...
def upload_clip_to_r2(local_path: str) -> str:
    ...
    client.upload_file(local_path, os.environ["R2_BUCKET"], key, ExtraArgs={"ContentType": "video/mp4"})
    return key


@app.function(gpu="H100", timeout=1800, volumes={"/weights": weights_volume},
...
def generate_clip(image_url: str, prompt: str) -> dict[str, object]:
    ...
    pipeline = LTX2Pipeline.from_pretrained(MODEL_ID, dtype=torch.bfloat16).to("cuda")
    weights_volume.commit()
    loaded = time.monotonic()

    video, audio = pipeline(
        image=load_image(image_url), prompt=prompt, negative_prompt=DEFAULT_NEGATIVE_PROMPT,
        width=960, height=544, num_frames=CLIP_FRAMES, frame_rate=float(CLIP_FPS),
        sigmas=DISTILLED_SIGMA_VALUES, guidance_scale=1.0, output_type="np", return_dict=False,
    )
    local_path = f"/tmp/{uuid.uuid4()}.mp4"
    encode_video(video[0], fps=CLIP_FPS, output_path=local_path, audio=audio[0].float().cpu(),
